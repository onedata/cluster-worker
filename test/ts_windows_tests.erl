...
-define(GET_ALL(Windows), ts_windows:get(Windows, undefined, #{})).
...
add_new_measurement_test() ->
    ...
    Test1 = ts_windows:aggregate(Windows, Timestamp, 1, sum),
    ?assertEqual({1, 1}, ts_windows:get_value(Timestamp, Test1)),

    Test2 = ts_windows:aggregate(Windows, Timestamp, 1, max),
    ?assertEqual(1, ts_windows:get_value(Timestamp, Test2)),

    Test3 = ts_windows:aggregate(Windows, Timestamp, 1, min),
    ?assertEqual(1, ts_windows:get_value(Timestamp, Test3)),

    Test4 = ts_windows:aggregate(Windows, Timestamp, 1, last),
    ?assertEqual(1, ts_windows:get_value(Timestamp, Test4)),

    Test5 = ts_windows:aggregate(Windows, Timestamp, 1, first),
    ?assertEqual(1, ts_windows:get_value(Timestamp, Test5)).


add_with_existing_timestamp_test() ->
    Timestamp = 1,
    Windows = ts_windows:aggregate(ts_windows:init(), Timestamp, 2, max),

    Test1 = ts_windows:aggregate(Windows, Timestamp, 3, max),
    ...
    Test3 = ts_windows:aggregate(Windows, Timestamp, 1, min),
    ?assertEqual(1, ts_windows:get_value(Timestamp, Test3)),
    Test4 = ts_windows:aggregate(Windows, Timestamp, 3, min),
    ?assertEqual(2, ts_windows:get_value(Timestamp, Test4)),

    Test5 = ts_windows:aggregate(Windows, Timestamp, 3, last),
    ?assertEqual(3, ts_windows:get_value(Timestamp, Test5)),

    Test6 = ts_windows:aggregate(Windows, Timestamp, 3, first),
    ?assertEqual(2, ts_windows:get_value(Timestamp, Test6)),

    WindowsWithSum = ts_windows:aggregate(ts_windows:init(), Timestamp, 2, sum),
    Test7 = ts_windows:aggregate(WindowsWithSum, Timestamp, 3, sum),
...
prune_overflowing_windows_test() ->
    Timestamp1 = 1,
    Timestamp2 = 2,
    Windows1 = ts_windows:aggregate(ts_windows:init(), Timestamp1, 1, max),
    Windows2 = ts_windows:aggregate(Windows1, Timestamp2, 1, max),

    Test1 = ts_windows:prune_overflowing_windows(Windows2, 3),
    ?assertEqual(Windows2, Test1),

    Test2 = ts_windows:prune_overflowing_windows(Test1, 2),
    ?assertEqual(Windows2, Test2),

    Test3 = ts_windows:prune_overflowing_windows(Test2, 1),
    ?assertEqual(undefined, ts_windows:get_value(Timestamp1, Test3)),
    ?assertEqual(1, ts_windows:get_value(Timestamp2, Test3)),

    Test4 = ts_windows:prune_overflowing_windows(Test3, 1),
    ?assertEqual(Test3, Test4),

    Test5 = ts_windows:prune_overflowing_windows(Test4, 0),
...
get_test() ->
    ...
    Windows = lists:foldl(fun({Timestamp, Value}, Acc) ->
        ts_windows:aggregate(Acc, Timestamp, Value, max)
    ...
    ?assertEqual(?GET_ALL_RESULT(ReversedMeasurements), ts_windows:get(Windows, MeasurementsCount, #{})),

    ?assertEqual(?GET_ALL_RESULT(lists:sublist(ReversedMeasurements, 3, MeasurementsCount - 2)),
        ts_windows:get(Windows, MeasurementsCount - 2, #{})),

    ?assertEqual({ok, []}, ts_windows:get(Windows, MeasurementsCount - 2, #{limit => 0})),
    ?assertEqual({{continue, #{limit => 1}}, lists:sublist(ReversedMeasurements, 3, MeasurementsCount - 2)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => MeasurementsCount - 1})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 5)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => 5})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 8)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => 8})),

    ?assertEqual({ok, []}, ts_windows:get(Windows, MeasurementsCount - 2, #{stop => MeasurementsCount - 1})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 1)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{stop => MeasurementsCount - 2})),
    ?assertEqual({{continue, #{stop => 0}}, lists:sublist(ReversedMeasurements, 3, MeasurementsCount - 2)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{stop => 0})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, MeasurementsCount - 2)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{stop => 1})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 4)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{stop => 5})),

    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 4)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => 5, stop => 5})),
    ?assertEqual({ok, lists:sublist(ReversedMeasurements, 3, 5)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => 5, stop => 1})),
    ?assertEqual({{continue, #{limit => 2, stop => 0}}, lists:sublist(ReversedMeasurements, 3, MeasurementsCount - 2)},
        ts_windows:get(Windows, MeasurementsCount - 2, #{limit => MeasurementsCount, stop => 0})).
...
split_test() ->
    ...
    SplitAns = ts_windows:split_windows(Windows, 3),
...
reorganization_test() ->
    ...
    ?assertEqual(true, ts_windows:should_reorganize_windows(Windows1, MeasurementsCount - 1)),
    ?assertEqual(false, ts_windows:should_reorganize_windows(Windows1, MeasurementsCount)),
    ?assertEqual(false, ts_windows:should_reorganize_windows(Windows1, MeasurementsCount + 1)),

    Test1 = ts_windows:reorganize_windows(Windows1, Windows2, 10, 1),
    ...
    Test2 = ts_windows:reorganize_windows(Windows1, Windows2, 20, 1),
    ...
    Test3 = ts_windows:reorganize_windows(Windows1, Windows2, 13, 1),
