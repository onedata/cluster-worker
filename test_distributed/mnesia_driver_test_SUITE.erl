...
get_should_return_doc(Config) ->
    Ctx = ?config(ctx, Config),
    {ok, Doc} = mnesia_driver:save(Ctx, ?DOC),
    ?assertEqual({ok, Doc}, mnesia_driver:get(Ctx, ?KEY)).

get_should_return_missing_error(Config) ->
    Ctx = ?config(ctx, Config),
    ?assertEqual({error, key_enoent}, mnesia_driver:get(Ctx, ?KEY)).
...
delete_should_remove_doc(Config) ->
    Ctx = ?config(ctx, Config),
    ?assertMatch({ok, _}, mnesia_driver:save(Ctx, ?DOC)),
    ?assertEqual(ok, mnesia_driver:delete(Ctx, ?KEY)),
    ?assertEqual({error, key_enoent}, mnesia_driver:get(Ctx, ?KEY)).
...
save_get_delete_should_return_success_base(Config) ->
    ...
    lists:foreach(fun(N) ->
        ?assertMatch(ok, mnesia_driver:delete(Ctx, ?KEY(N)))
    end, lists:seq(1, OpsNum)).
...
init_per_testcase(Case, Config) ->
    Ctx = #{table => Case},
    ?assertMatch({ok, _}, application:ensure_all_started(mnesia)),
    ?assertEqual(ok, mnesia_driver:init(Ctx, [])),
    [{ctx, Ctx} | Config].
