...
%%% environment where task was created while executor represents environment where task will be processed.
...
%%% is a result of the fact that task can be scheduled for other environment thus main job must be available there
%%% (all other jobs will be created on environment that processes task).
...
%%% Job handling functions are provided by callback modules (see traverse_job_behaviour.erl). Callback modules provide also
...
-type description() :: #{atom() => integer()}.  % map describing progress of task; framework provides following counters:
                                                ...
                                                % the user can add own counters returning map with value upgrade from job
                                                % (see traverse_job_behaviour.erl)
...
%%--------------------------------------------------------------------
%% @doc
%% Checks if there is task to be executed and starts it if possible.
%% @end
%%--------------------------------------------------------------------
