...
init_tree(Ctx, Key, TreeId, Batch, ReadOnly) ->
    ...
    case Ans of
        ...
        {broken_root, Tree} ->
            ...
            ?THROTTLE_LOG(Key, TreeId, ?error("Broken bp_tree~ts", [?autoformat([TreeId, Key, Ctx])])),
            {ok, Tree};
        {{error, interrupted_call} = Error, Tree} ->
            ?THROTTLE_LOG(Key, TreeId, ?warning("Interrupted call (tree init)~ts", [?autoformat([TreeId, Key, Ctx])])),
            case Ctx of
