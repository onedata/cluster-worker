...
-export([time_series_collection_list/3, infinite_log_operation/4]).
...
-spec time_series_collection_list(node(), datastore_doc:ctx(), list()) ->
    ok | {ok, [ts_windows:window()] | time_series_collection:windows_map()} | {error, term()}.
time_series_collection_list(FetchNode, Ctx, Args) ->
    try
        ListResult = case Args of
            [Id, Options] ->
                time_series_collection:list(set_direct_access_ctx(FetchNode, Ctx), Id, Options, undefined);
            [Id, RequestedMetrics, Options] ->
                time_series_collection:list(set_direct_access_ctx(FetchNode, Ctx), Id, RequestedMetrics, Options, undefined)
        end,
        case ListResult of
            {{ok, Result}, _} ->
                {ok, Result};
        ...
        throw:{fetch_error, not_found} ->
            datastore_router:execute_on_node(FetchNode, datastore_writer, time_series_collection_operation, [Ctx, list, Args])
    end.
