...
%% API
-export([route/2, route_histogram_operation/2, route_infinite_log_operation/2, execute_on_node/4]).
...
-spec route_histogram_operation(atom(), list()) -> term().
route_histogram_operation(get, [Ctx | Args]) ->
    route_internal(datastore_reader, histogram_get, Ctx, [Args]);
route_histogram_operation(Function, [Ctx | Args]) ->
    route_internal(datastore_writer, histogram_operation, Ctx, [Function, Args]).
