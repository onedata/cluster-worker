...
%%% The basic concept are tasks (see traverse_task.erl) and pools. Each task represents single traverse while pool
%%% represents single execution environment. The tasks can be scheduled to be executed on different clusters so
%%% a concept of executor is introduced (executor represents single cluster).
%%% The task is processed executing jobs. The job represents processing of single entity (e.g., dir or file).
%%% First job (called main_job) is provided to task start function, further jobs can be generated as first and next jobs result.
%%% First job is treated specially as it is the only job that exists before task start. Thus, its id is stored in a task
%%% document (see traverse_task.erl) and its special status is highlighted when it is persisted. Such special treatment
%%% is a result of the fact that task can be scheduled for other executor thus main job must be available there
%%% (all other jobs will be created by executor that processes task).
%%% The jobs are divided into two groups: master and slave jobs. Master jobs precess entities that that define structure
...
%%% Worker pools can be started on several nodes. In such a case, load is balanced between nodes executing different tasks
%%% on different nodes (all jobs connected with single task are executed on the same node - see traverse_load_balance.erl).
%%% Job handling functions are provided by callback modules (see job_behaviour.erl). Callback modules provide also
%%% functions needed for jobs persistency (master jobs are stored only). They also can provide additional information
%%% for datastore documents synchronization and tasks sorting (see traverse_task_list). Multiple callback modules can
%%% be used for single pool (different callback modules for different tasks).
%%% @end
%%%-------------------------------------------------------------------
-module(traverse).
-author("Michal Wrzeszcz").
...
-type executor() :: traverse_task_list:tree(). % entity (cluster) that processes tasks
-type pool_options() :: #{
    executor => executor(),
...
-type description() :: #{atom() => integer()}.  % map describing progress of task; framework provides following counters:
                                                ...
                                                % the user can add own counters returning map with value upgrade/decrease from job
...
-type group() :: binary(). % group used for load balancing
-type run_options() :: #{
    executor => executor(),
    creator => executor(),
...
run(PoolName, TaskID, Job, Options) ->
    ...
    {JobStatus, Node, Description} = case Creator =:= Executor of
        true ->
            case traverse_load_balance:increment_ongoing_tasks(PoolName) of
                {ok, ChosenNode} ->
        ...
        try
            ...
            E2:R2 ->
                ?error_stacktrace("Checking finish of job ~p of task ~p (module ~p) error ~p:~p",
                    [Job, TaskID, CallbackModule, E2, R2])
        end
    catch
        E1:R1 ->
            ?error_stacktrace("Master job ~p of task ~p (module ~p) error ~p:~p",
                [Job, TaskID, CallbackModule, E1, R1]),
            ErrorDescription = #{
...
check_task_list_and_run(PoolName, Executor) ->
    case traverse_load_balance:get_next_group(PoolName) of
        {error, no_groups} ->
            ok = traverse_load_balance:decrement_ongoing_tasks(PoolName);
        {ok, GroupID} ->
    ...
    case traverse_load_balance:increment_ongoing_tasks(PoolName) of
        {ok, Node} ->
            ...
            case traverse_task:start(ExtendedCtx, PoolName, TaskID, #{master_jobs_delegated => 1}) of
                ...
                {error, already_started} ->
                    traverse_load_balance:decrement_ongoing_tasks(PoolName)
            end
    end.

-spec cancel_group(pool(), group(), executor()) -> ok| {abort, traverse:id()}.
cancel_group(PoolName, Group, Executor) ->
    case traverse_load_balance:deregister_group(PoolName, Group) of
        ok ->
